...
void avx_transpose(int *src, int *dst, int w, int h)
{
    for (int x = 0; x < w; x += 8) {
        for (int y = 0; y < h; y += 8) {
            ...
            _mm256_storeu_si256((__m256i *)(dst + ((x + 4) * h) + y), I4);
            _mm256_storeu_si256((__m256i *)(dst + ((x + 5) * h) + y), I5);
            _mm256_storeu_si256((__m256i *)(dst + ((x + 6) * h) + y), I6);
            _mm256_storeu_si256((__m256i *)(dst + ((x + 7) * h) + y), I7);
        }
    }
}
...
void avx_prefetch_transpose(int *src, int *dst, int w, int h)
{
    for (int x = 0; x < w; x += 8) {
        for (int y = 0; y < h; y += 8) {
#define PFDIST_AVX  8
            _mm_prefetch(src+(y + PFDIST_AVX + 0) *w + x, _MM_HINT_T1);
            _mm_prefetch(src+(y + PFDIST_AVX + 1) *w + x, _MM_HINT_T1);
            _mm_prefetch(src+(y + PFDIST_AVX + 2) *w + x, _MM_HINT_T1);
            _mm_prefetch(src+(y + PFDIST_AVX + 3) *w + x, _MM_HINT_T1);
            _mm_prefetch(src+(y + PFDIST_AVX + 4) *w + x, _MM_HINT_T1);
            _mm_prefetch(src+(y + PFDIST_AVX + 5) *w + x, _MM_HINT_T1);
            _mm_prefetch(src+(y + PFDIST_AVX + 6) *w + x, _MM_HINT_T1);
            _mm_prefetch(src+(y + PFDIST_AVX + 7) *w + x, _MM_HINT_T1);
